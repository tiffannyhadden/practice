...
/*function makeCar (input1, input2) {
    // console.log(JSON.parse('{"make": input1, "model": input2}'))
    // let input1Stuff =
    return JSON.parse(`{ "name":` + input1 + "," +  `"age":` + input2 + `}`);
    // return JSON.parse('{"make": ' + input1, + '"model":input2}');
}
...
// function getProfileCount(profiles) {
...
// }
//
// console.log(getActiveProfiles(profiles))
//  getInactiveCount() should return the number of inactive profiles
function getInactiveCount(array) {
    return array.reduce(
...
function sumOfAllBalances(array) {
    let sum = 0;
    for(let i = 0; i < profiles.length; i++) {
        // str.substring(1)
        sum += JSON.parse(profiles[i].balance.substring(1).replace(/,/g, ''));
    }
    return sum;
}
...
console.log("THIS ONE " + getLowestBalance(profiles))
